...
class Asset: Identifiable, Hashable, Equatable {
    ...
    init(bundleResource name: String, withExtension ext: String) {
        self.id = "\(name).\(ext)"
...
extension Asset {
    ...
    static func loadFromBundle(resource name: String, withExtension ext: String) -> Asset? {
        ...
        if let source = CGImageSourceCreateWithURL(url as CFURL, nil),
           let thumbnail = loadThumbnail(from: source) {
            return Asset(file: url, thumbnail: thumbnail)
        } else {
